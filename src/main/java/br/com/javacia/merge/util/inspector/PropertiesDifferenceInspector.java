...
public class PropertiesDifferenceInspector implements DifferenceInspector {

	private Properties propLocal;
	private Properties propRemote;

	public PropertiesDifferenceInspector(File fileLocal, File fileRemote) {
		propLocal = loadProperties(fileLocal);
		propRemote = loadProperties(fileRemote);
	}

	public PropertiesDifferenceInspector(Reader readerLocal, Reader readerRemote) {
	...
	public List<Difference> findDifferences() {
		...
		for (String name : keyLocal) {
			...
			} else if (!valueRemote.trim().equals(valueLocal.trim())) {
				differences.add(new Difference(new PropertiesType(PropertiesTypes.PROPERTIES), name, valueLocal, valueRemote));
			}
